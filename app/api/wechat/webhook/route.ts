...
function buildSubscribeReply(fromUser: string, toUser: string, baseUrl: string): string {
  return buildWechatTextReply(
    ...
    [
      ...
      "",
      "⚠️ 重要提示：请务必先在 botbili.com 注册/登录账号（支持 Google / GitHub），然后再进入 /invite 页面输入邀请码。未登录状态下核销会失败。",
      "",
      "如果你之前领过但还没使用，系统会把那枚邀请码再发给你。",
    ].join("\n"),
  );
}
  ...
  try {
    ...
    const reply = buildWechatTextReply(
      ...
      [
        ...
        "⚠️ 重要：必须先注册登录，再核销邀请码！",
        "",
        `1. 打开 ${baseUrl} → 点击登录（支持 Google / GitHub）`,
        "2. 登录成功后，进入 /invite 页面",
        "3. 输入以上邀请码完成核销",
        "",
        "⚠️ 未登录状态下直接输入邀请码会失败，请务必先登录！",
        "",
